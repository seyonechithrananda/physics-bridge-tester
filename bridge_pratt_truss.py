...
def createPratt(length, height, n):
    dx = length/n
    x = [i*dx for i in range(n+1)] + [i*dx for i in range(1,n)] # x coordinates of nodes
    y = [0 for i in range(n+1)] + [height for i in range(1,n)] # y coordinates of nodes (first set: y=0, second set: y=height)
    connections = []
    for i in range(1, n):
...
def createKTruss(length, height, n):
    ...
    connections = []
    for i in range(1,n):
        connections.append([i, i+2*n-1]) # connect base nodes
    connections.append([0,2*n])
    connections.append([n, 3*n-2])
    connections.extend([[i, i+1] for i in range(n)])
    connections.extend([[i, i+1] for i in range(2*n, 3*n-2)])
    for i in range(n+1, n+math.ceil(n/2)): # diagonal truss structures
        connections.append([i, i+n])
        connections.append([i, i-n+1])
